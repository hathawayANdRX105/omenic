...
本指南指导 agent 开发多步 PR 工作（依赖感知、隔离、委派、验证、审查、清理）。检查由 `.githooks/hooks` 强制。
...
按 spec/github_reviews.yaml：

| 类型 | 格式 | 位置 |
|------|------|------|
| CRG Review | `## Agent 🤖 - CRG Review: <英文标题>`（H2），子分类 `###`（H3），正文中文 | PR conversation |
| Inline Review | `Agent 🤖 - Inline Review P0|P1|P2|P3: <内容>`，锚定 path+line | Files changed |
| Reply | `Agent 🤖 - Fix: <原因>` 或 `Block:` / `Note:` / `Resolve:` / `Withdraw:` / `Supersede:` | 原 thread 回复 |

**禁 checkbox**：review 评论中严禁 `- [x]` / `- [ ]`（P-22 → FAIL）。

### 提交方式

```bash
# CRG Review（PR conversation 评论）
gh pr comment <N> --body "## Agent 🤖 - CRG Review: <title>\n\n### 分类\n内容"

# Inline Review（review 提交，含 comment thread）
gh api repos/<owner>/<repo>/pulls/<N>/reviews \
  -f event=COMMENT \
  -f body="CRG Review 汇总" \
  -f 'comments[][path]=path/to/file' \
  -F 'comments[][line]=42' \
  -f 'comments[][body]=Agent 🤖 - Inline Review P2: 内容'
```
...
- 走 gh-gate 创建 PR（`python .githooks/gh-gate pr create ...`）
...
合并后重读 PR 状态、merge commit、issue 状态、checkbox——不靠命令输出来推断。
...
- [ ] 走 gh-gate 创建 PR，创建后校验 ALL PASS

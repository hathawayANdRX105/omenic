...
use agent_loop::compaction::{
    CharBudgetPolicy, CompactionPolicy, KEEP_RECENT_CHARS, Message, NoopSummarizer,
};
...
impl agent_loop::compaction::Summarizer for Always {
...
fn kept_window_alone_over_budget_is_kept() {
    ...
    let total: usize = msgs.iter().map(agent_loop::compaction::message_chars).sum();

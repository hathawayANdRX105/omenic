...
use adaptor::{Content, Context, Message, Model, StopReason, StreamEvent, ToolCallSpec};
...
fn container(
    ...
    Arc<omenic_harness_tools::ToolCatalog>,
    Arc<omenic_harness_compaction::CharBudgetPolicy>,
    usize,
) {
    let (fiber, _registry) = omenic_composition::assemble(doc, vec![]).expect("assemble");
    let catalog = fiber
...
fn pairing_ids(
    ...
    for m in msgs {
        ...
        for b in bs {
            match b {
                adaptor::Block::ToolUse { id, .. } => {
                    uses.insert(id.clone());
                }
                adaptor::Block::ToolResult { tool_use_id, .. } => {
...
fn container_policy_drives_compaction_without_breaking_pairs() {
    ...
    assert_eq!(budget, omenic_harness_compaction::COMPACT_CHAR_BUDGET);
    ...
    assert!(
        matches!(
            ctx.messages.first(),
            Some(m) if matches!(&m.content, Content::Text(s) if s.starts_with(omenic_harness_compaction::SUMMARY_PREFIX))
        ),

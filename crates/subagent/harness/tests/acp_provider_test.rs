...
fn start(spec: &AcpProviderSpec, prompt: &str) -> subagent_harness::SubagentRun {
...
fn settle_then_dispose(run: subagent_harness::SubagentRun) -> SubagentResult {
...
fn dispose_then_settle(run: subagent_harness::SubagentRun) -> SubagentResult {
    bounded(SETTLE_TIMEOUT, move || {
        run.dispose();
        run.result()
    })

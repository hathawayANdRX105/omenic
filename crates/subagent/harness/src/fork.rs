...
/// `subagent::runner::run_subagent` with a read-only tool set and reports the
...
impl SubagentProvider for ForkProvider {
    ...
    fn start(&self, request: SubagentStartRequest) -> crate::provider::SubagentRun {
        ...
        let join = thread::spawn(move || {
            let outcome = subagent::runner::run_subagent(
                backend.as_ref(),
                &model,
                &prompt,
                max_turns,
            ...
            let result = match outcome {
                ...
                Err(subagent::SubagentError::Aborted { .. }) => SubagentResult::Aborted,

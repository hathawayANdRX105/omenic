...
pub struct OrbitConfig {
    ...
    pub compaction: std::sync::Arc<compaction::CharBudgetPolicy>,
    /// Tool catalog (`harness.tools`), adapted onto orbit's tool trait at
    /// the seam — see [`orbit_tools`].
    pub catalog: std::sync::Arc<tools_harness::ToolCatalog>,
...
struct HarnessTool {
    ...
    description: String,
    parameters: Value,
    inner: std::sync::Arc<dyn tools_harness::Tool>,
...
impl HarnessTool {
    fn new(
        tool: std::sync::Arc<dyn tools_harness::Tool>,
        abort: std::sync::Arc<std::sync::atomic::AtomicBool>,
    ) -> Self {
        let spec = tool.spec();
        HarnessTool {
...
fn orbit_tools(
    catalog: &tools_harness::ToolCatalog,
    abort: std::sync::Arc<std::sync::atomic::AtomicBool>,
...
struct OrbitEngine {
    ...
    compaction: std::sync::Arc<compaction::CharBudgetPolicy>,

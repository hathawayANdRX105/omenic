...
fn orbit_setup_with_mcp_tools_stays_clone_and_shares_the_list() {
    ...
    let setup = OrbitSetup {
        ...
        config: OrbitConfig {
            cwd: None,
            max_turns: 8,
            compaction: Arc::new(compaction::CharBudgetPolicy::default()),
...
fn real_session_tools_register_every_name_in_the_constant() {
    ...
    let built = tools_harness::jobs_terminal::session_tools(jobs, terminals);

    let mut built_names: Vec<String> = built.iter().map(|t| t.name().to_string()).collect();
    let mut declared: Vec<String> = tools_harness::jobs_terminal::SESSION_TOOL_NAMES
        .iter()
        .map(|s| s.to_string())
        .collect();

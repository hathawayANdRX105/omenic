...
if [[ $n_inline -eq 0 ]]; then
  ...
  n_unanchored=$(echo "$inline_json" | jq '[.[] | select(.line == null)] | length')
  ...
  # P-22 inline 无确认噪声
  n_noise=$(echo "$inline_json" | jq '[.[] | select(.in_reply_to_id == null and (.body | test("确认无问题|没问题|无问题|OK"))) ] | length')
  ...
  # P-23 [Agent 🤖]: 前缀（每条 comment 必须以 [Agent 🤖] 开头）
  bad_prefix=$(echo "$inline_json" | jq '[.[] | select((.body | startswith("[Agent 🤖]")) | not)] | length')
  if [[ $bad_prefix -eq 0 ]]; then report PASS P-23 "all inline comments use [Agent 🤖]: prefix";
  else report WARN P-23 "$bad_prefix inline comments lack [Agent 🤖]: prefix"; fi
fi

# ============ 2.7 inline thread 闭环 ============
echo "--- inline thread closure ---"
# 计算根 finding 数量与各自 reply
root_count=$(echo "$inline_json" | jq '[.[] | select(.in_reply_to_id == null)] | length')
replied_root=0
for id in $(echo "$inline_json" | jq -r '.[] | select(.in_reply_to_id == null) | .id'); do
  reply_body=$(echo "$inline_json" | jq -r --arg id "$id" '[.[] | select(.in_reply_to_id == ($id|tonumber)) | .body] | join("\n")')
  if [[ -n "$reply_body" ]] && grep -qE '\[Agent 🤖\] (已修复|已解决|观察项，不阻塞)' <<<"$reply_body"; then
    replied_root=$((replied_root+1))
  elif [[ -n "$reply_body" ]]; then
    : # 有 reply 但前缀不对 → P-25 处理
  fi
done
...
sha_missing=$(echo "$inline_json" | jq '[.[] | select(.in_reply_to_id != null and (.body | test("[0-9a-f]{7,40}") | not))] | length')
if [[ $replied_root -eq 0 ]] && [[ $sha_missing -eq 0 ]]; then report PASS P-25 "no replies to check";
elif [[ $sha_missing -eq 0 ]]; then report PASS P-25 "all fix replies cite commit SHA";
else report WARN P-25 "$sha_missing fix replies lack commit SHA"; fi
...
issue_num=$(echo "$pr_body" | grep -oE 'Fixes #([0-9]+)' | grep -oE '[0-9]+' | head -1)
if [[ -n "$issue_num" ]]; then
